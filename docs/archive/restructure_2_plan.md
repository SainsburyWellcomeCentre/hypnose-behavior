...
| 7b.4b step 3 — port the readers onto `position_data` | **done** 2026-08-13 | `HEAD` | **13 call sites across 6 modules, blobs still present, nothing drawn differently.** One new primitive, `frames.position_entries_by_trial(position_data, flag)` -> `{global_trial_id: [entry, ...]}` sorted by position, which **reduces nothing**: the callers disagree on the rule — `sing_rew_movement` takes the **max** `poke_odor_end`, `movement.py` and `_last_poke_out_scanning_back` scan **back by position**, `_last_poke_out_by_position` takes the **last entry and accepts its null** — and sections 13/14 say do not merge helpers that differ, so all three survive verbatim. **Every site passes the provenance flag matching the blob it used to read** (section 2); `_last_valve_start` is where that bites, since `position_valve_times` is a superset and an unfiltered join would gain positions the poke blob never had. New seam `io/loaders._load_position_data(results_dir, trials)` so **step 4 repoints one function, not nine call sites** — it takes `trials` because `sing_rew._session_reward_rts` passes a *filtered subset*, which a naive file read would silently widen. Five dead blob parsers deleted (`_extract_position_entry`, `_ordered_position_entries`, `prep._last_position_entry` — imported twice, called never — `_parse_like_mapping`, `_iter_position_entries`) plus two now-unused `json` imports. **`compute_speed_analysis`'s four reads have no gate and can have none** (notebook-only caller; it *writes* `speed_analysis.parquet` into the read-only share), so its control is an AST-lift of `_last_poke_out` / `_last_valve_start` from **both** trees over all 9 sessions: **1731/1731 identical for both, on the written parquet and the load-time derivation.** **Trap: the first sensitivity check reported the probe BLIND — and correctly**, because the perturbed cell was one neither helper *reads*; re-aimed at the returned cell it discriminates cleanly (one cell → `differs:1`; every `poke_odor_end` +1 ms → 272/273 with **0** on `valve_start`, and vice versa). **A probe validated with the wrong perturbation proves nothing** — section 27's lesson in a second costume. **Gates: `plot_regression` GREEN 38/38 COUNTED** (including all three cases added in step 1b, which is what made the port checkable at all), **`regression` GREEN 54/54 no regeneration**, `check_imports` PASS. `verify_scripts` skipped (no CLI wiring touched). `DECISIONS.md` section 28 |
| 7b.4b step 4 — the loaders read the side-table | **done** 2026-08-13 | `HEAD` | **`position_data.parquet` becomes the source; the derivation becomes the compatibility path.** One definition, `io/load_results.load_position_data(results_dir, trials)`, **re-exported** by `io/loaders.py` under the private name the readers already import — the `readers.py` arrangement, so there is one definition site and `loaders`' heavy `harp`/`aeon` imports stay out of the light `load_results`. Step 3's seam paid off: the switch was **one function**, plus repointing the eleven `build_position_data(loaded_td)` sites the phase brief had not inventoried (two needed plumbing — `_extract_odor_poke_ms` and `_session_reward_rts` gained a `results_dir` from their callers). **Always filtered back to `trials`**, because `sing_rew._session_reward_rts` passes a *rewarded-only subset* and an unfiltered read would widen it to the whole session with no error; when the frames cannot be keyed on `global_trial_id` the derivation is used instead, since it cannot make that mistake. `SessionResults` keys off the `results_dir` **already in the mapping**, so section 5's laziness survives with no attribute to keep in step through `copy()`. **The neither-source case now warns** (blobs gone *and* no saved table = a session that missed the re-analysis) instead of handing back an empty frame every per-position metric reads as "no positions" — section 27's failure one level up. **Verified on all three paths before gating**: read taken (`datetime64[ns]` vs the derivation's `str`, so the switch is provably not a no-op), fallback fires (1,019 rows), neither → 0 rows + 1 warning; a 20-trial subset returns 59 rows spanning exactly 20 trials. **The dtype change moved nothing drawn** — section 24 had measured only the *metrics* normalise through `_tz_naive`, and the seven viz readers ported in step 3 are not metrics. **Gates: `regression` GREEN 54/54 no regeneration, `plot_regression` GREEN 38/38 COUNTED**, `check_imports` PASS. `DECISIONS.md` section 28 |
| 7b.4b step 5 — **drop the blobs** | **done** 2026-08-13 | `HEAD` | **Intended output change, fixtures regenerated in the same commit. `trial_data` is now one row per trial, scalar columns only.** The three blobs stay declared `TrialRecord` fields — `slots=True` would refuse `classify_trials`' assignment, and `save_results` still expands them into `position_data.parquet` — but `POSITION_BLOB_COLUMNS` states "in memory, not on disk" once, and `columns()` excludes them so the conform and the loader's schema check stop demanding a column nothing writes. **`mode_independent_columns()` had to be fixed with it**: it built its list from `fields()` rather than `columns()`, so an untagged file written by current code would have reported itself missing three columns — two spellings of one declaration, section 27's trap again. Section 22's subset invariant re-verified (57 ⊂ 58/70/63). **The load-bearing line: the drop happens on the way to the file, never from `classification`.** The first version mutated the returned dict — which `print_merged_session_summary` reads after the save (its per-position stats would have gone silently empty) and which **`position_data_lossless` itself** reads to source the blobs it compares. **That would have made the acceptance gate for the drop pass by having nothing left to check.** Confirmed non-vacuous from the counts: still 4,791 `(blob,key)` occurrences per pair, unchanged. **Acceptance exactly as specified: 9 `[RED]` on `trial_data`, each the identical `- removed column: position_poke_times, position_valve_times, presentations`; ZERO `+ added`, ZERO `~ changed`, zero `[ERROR]`; the other 45 fingerprints green** — every metric md5 (including the two reported ones re-plumbed in step 2) and all three side tables untouched. **Gates: `position_data_lossless` GREEN 9/9 non-vacuous, `plot_regression` GREEN 38/38 COUNTED, `regression` GREEN 54/54 after a deliberate `--generate`** (each fixture losing exactly three column keys and its `trial_data` md5), `check_imports` PASS. **A second server re-analysis is not required**: the drop affects only newly written files, and sessions written by 7b.6 keep their blobs harmlessly since the loader prefers the side-table. `DECISIONS.md` section 28 |
| 7b schema & formats | **done** 2026-08-13 | `HEAD` | **Phase 7 is closed.** `TrialRecord` is the single schema and the loader checks against it; `save_csv` optional with parquet for every table; `position_data.parquet` plus the two per-grain metric tables written; **the three JSON blobs removed from `trial_data`**; the manifest carries commit + version; the server's derivatives re-analysed; no pickle outputs. The gate grew **2 fingerprints → 6** and `plot_regression` **32 → 38 cases** across the phase |
