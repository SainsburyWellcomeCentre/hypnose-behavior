...
| 6b `poke_source` + the 0 ms positions | not started | | `DECISIONS.md` §10. **Intended output change**, fixtures regenerated in the same commit. 6a is done, so this is unblocked — and §14 gives it a second acceptance test: re-run `qc/outcome_agreement.py` afterwards and the one remaining conflict should be **gone** |
