...
| ↳ follow-up owed by 6b | not started | | The §15 response-time fallback still fires 20/20 — `analyze_response_times` builds its own position map from valve events and never sees `poke_source`. Repointing its target at the last `poke_source == 'poke'` position is a **separate intended change** with its own fixture regeneration. `DECISIONS.md` §15 |
