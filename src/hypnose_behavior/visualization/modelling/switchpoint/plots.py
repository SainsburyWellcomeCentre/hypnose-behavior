...
def _rolling_mean(s: np.ndarray, window: int = _ROLLING_WINDOW) -> tuple[np.ndarray, np.ndarray]:
    ...
    **Do not move this onto `visualization.primitives.rolling_mean`.** The convolution
    is a dot product; a windowed `np.mean` sums pairwise, and the two disagree in the
    last ULP on 66% of values at ``window=21``. Summation style is part of the quantity
    (DECISIONS.md section 1), and no `plot_regression` case exercises this module, so
    the drift would be caught by nothing. The primitive also anchors at the window's
    last element where this anchors at its centre.
    """
    if s.size < window:
